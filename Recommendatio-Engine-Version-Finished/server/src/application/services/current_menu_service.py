...
class CurrentMenuService:
    def __init__(self):
        self.current_menu_repository = CurrentMenuRepository()
    ...
    def get_current_menu(self):
        try:
            current_menu_items = self.current_menu_repository.get_current_menu_items()
    ...
    def sort_menu_items_by_preferences(self, menu_items, preferences):
    ...
    def get_sorted_menu_items_by_preferences(self, employee_id):
        user_preferences = self.profile_service.get_user_preferences(employee_id)
        if not user_preferences:
            return f"No preferences found for employee ID {employee_id}"

        current_menu_items = self.get_current_menu()
        if isinstance(current_menu_items, str):  
            return current_menu_items

        sorted_menu_items = self.sort_menu_items_by_preferences(current_menu_items, user_preferences)
    ...
    def choose_recommended_item(self, employee_id, item_id, time_of_day):
        try:
            if not self.validation_repository.is_valid_item_id(item_id):
                return f"Item ID {item_id} is not valid."
            if self.choice_repository.item_already_chosen(employee_id, item_id, time_of_day):
                return f"Item ID {item_id} has already been chosen for {time_of_day}."
            self.current_menu_repository.insert_choice(employee_id, item_id, time_of_day)
    ...
    def clear_current_menu(self):
        try:
            self.current_menu_repository.clear_current_menu()
    ...
    def add_item_to_current_menu(self, item_id):
        try:
            if not self.validation_repository.is_valid_item_id(item_id):
                return f"Item ID {item_id} is not valid."
            if self.current_menu_repository.is_item_in_current_menu(item_id):
                return f"Item ID {item_id} is already in the current menu."
            self.current_menu_repository.add_to_current_menu(item_id)
    ...
    def finalize_current_menu(self):
        try:
            new_menu_items = self.current_menu_repository.get_current_menu_items()
