...
class MenuValidator:
    ...
    def get_existing_item_id(self, prompt, fetcher_method):
        item_details = fetcher_method()
        item_ids = [item['id'] for item in item_details]
        while True:
            item_id = self.input_validator.get_valid_item_id(prompt)
    ...
    def get_existing_current_item_id(self, prompt, fetcher_method):
        item_details = fetcher_method()
        item_ids = [item['id'] for item in item_details]
        while True:
            item_id = self.input_validator.get_valid_item_id(prompt)
