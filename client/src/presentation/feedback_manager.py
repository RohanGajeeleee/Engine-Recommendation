...
class FeedbackManager:
    ...
    def add_feedback(self, employee_id, current_date):
        ...
        request = FeedbackRequestBuilder.add_feedback_request(employee_id, item_id, rating, comment, time_of_day, current_date)
        response = send_request(request)
        print(response)
        return True

    def view_feedback(self):
        request = FeedbackRequestBuilder.view_feedback_request()
        response = send_request(request)
        print(response)
        return True
