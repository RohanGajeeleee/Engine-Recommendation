...
class Report:
    ...
    def generate_monthly_feedback_report(year, month):
        ...
        JOIN
            users u ON f.employee_id = u.employee_id
        WHERE 
    ...
    def _generate_and_save_report(data, year, month):
        ...
        with open(filename, 'w') as file:
            file.write("Menu ID,Menu Name,Employee Name,Comment,Rating,Feedback Date\n")
            for row in data:
                file.write(f"{row['menu_id']},{row['menu_name']},{row['employee_name']},{row['comment']},{row['rating']},{row['feedback_date']}\n")
        print(f"Report saved to {filename}")
        print("Monthly Feedback Report")
        for row in data:
            print(f"Menu ID: {row['menu_id']}, Menu Name: {row['menu_name']}, Employee Name: {row['employee_name']}, Comment: {row['comment']}, Rating: {row['rating']}, Date: {row['feedback_date']}")
